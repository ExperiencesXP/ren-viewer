...
class GameVFS:
    ...
    def rebuild(self) -> None:
        tmp = GameVFS(
            ...
            lone_only=self.lone_only,
        )
        tmp._populate()
        with self._lock:
            self._index = tmp._index
            self._basedir_files = tmp._basedir_files
            self.archives = tmp.archives
    ...
    def source(self, logical: str) -> FileSource | None:
        with self._lock:
            return self._source_unlocked(logical)

    def _source_unlocked(self, logical: str) -> FileSource | None:
    ...
    def _pin_source(self, logical: str) -> tuple[FileSource, RpaArchive | None]:
        with self._lock:
            src = self._source_unlocked(logical)
            if src is None:
    ...
    def read(self, logical: str) -> bytes:
        src, arch = self._pin_source(logical)
        if src.layer == "archive":
    ...
    def read_range(self, logical: str, start: int, length: int) -> bytes:
        src, arch = self._pin_source(logical)
        if start < 0 or length <= 0 or start >= src.length:
            return b""
        length = min(length, src.length - start)
        if src.layer == "archive":
            assert arch is not None
            return arch.read_range(src.path, start, length)
        if src.disk_path:
            with Path(src.disk_path).open("rb") as fh:
                fh.seek(start)
    ...
    def tree(self, prefix: str = "") -> list[VfsNode]:
        ...
        with self._lock:
            ...
            basedir_items = list(self._basedir_files.items())
