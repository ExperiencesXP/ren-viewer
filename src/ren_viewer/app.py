...
def create_app() -> FastAPI:
    ...
    def api_file(path: str, request: Request, download: bool = False):
        session = _require()
        if not session.vfs.exists(path):
            raise HTTPException(404, path)
        size = session.vfs.stat_size(path)
        try:
        ...
        start, end = span
        data = session.vfs.read_range(path, start, end - start + 1)
        head = data if start == 0 else session.vfs.read_range(path, 0, min(64 * 1024, size))
        info = sniff(head, path)
        return Response(
            content=data,
            status_code=206,
            ...
            headers={
                "Accept-Ranges": "bytes",
                "Content-Range": f"bytes {start}-{start + len(data) - 1}/{size}" if data else f"bytes */{size}",
    ...
    def api_script_content(path: str):
        ...
        try:
            ...
            if not has_session():
                raise HTTPException(409, "no game open") from exc
            raise
        return {
            "path": path,
            "text": text,
    ...
    def api_gallery():
        session = _require()
        status = session.index_status()
        idx = session.index
        return {
            "gallery_mentioned": bool(idx.gallery_mentioned) if idx is not None else False,
            "items": session.gallery_items(),
            "index": status,
        }
...
class _UnsatisfiableRange(Exception):
    """HTTP Range is outside the file."""


def _byte_range(header: str | None, size: int) -> tuple[int, int] | None:
    """Inclusive start/end, or None to send the whole file."""
