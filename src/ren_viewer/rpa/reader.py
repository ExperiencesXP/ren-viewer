...
class RpaArchive:
    ...
    def read_range(self, logical_path: str, start: int, length: int) -> bytes:
        segs = self.entries.get(_norm(logical_path))
        if segs is None:
            raise FileNotFoundError(logical_path)
        ...
        with self.path.open("rb") as fh:
            for seg in segs:
                plen = len(seg.prefix)
                if plen:
                    if cursor + plen > start and cursor < end:
                        lo = max(0, start - cursor)
                        hi = min(plen, end - cursor)
                        out.extend(seg.prefix[lo:hi])
                    cursor += plen
                    if cursor >= end:
                        break
                if seg.length:
                    if cursor + seg.length > start and cursor < end:
                        lo = max(0, start - cursor)
                        hi = min(seg.length, end - cursor)
                        fh.seek(seg.offset + lo)
                        chunk = fh.read(hi - lo)
                        if len(chunk) != hi - lo:
                            raise OSError(f"short read for {logical_path}")
                        out.extend(chunk)
